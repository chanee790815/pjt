...
# 2. 프로젝트 상세 관리 (I2, J2 업데이트 및 이력 저장 완전 보전)
def view_project_detail(sh, pjt_list):
    ...
    if selected_pjt != "선택":
        ...
        with col_pm2: 
            st.write("")
            # [수정] IndentationError 해결: block 내부 들여쓰기 정렬
        ...
        with tab1:
            try:
                ...
                if not cdf.empty:
                    ...
                    max_width = cdf['구분_고유'].apply(ctw).max()
                    cdf['구분_고유'] = cdf['구분_고유'].apply(lambda x: str(x) + "&nbsp;" * int((max_width - ctw(x)) * 2.5))
        ...
        with tab3:
            ...
            try:
                ...
                if not h_data.empty:
                    p_match = h_data[h_data['프로젝트명'].astype(str).str.strip() == selected_pjt.strip()]
...
if check_login():
    ...
    if client:
        try:
            ...
            # 메뉴 목록에 PPT 자동 생성 추가 (외부 ppt_engine 모듈 호출)
            ...
            elif menu == "📊 PPT 자동 생성": ppt_engine.view_ppt_generator(DEFAULT_TEMPLATE)
