...
## 버전: Rev. 0.5.4 (Final Stable Release)
## 업데이트 요약:
## 1. 🔐 보안: Secrets 기반 멀티 계정 로그인 (ID/PW 인증)
## 2. 🏠 대시보드: 모든 프로젝트 진행률 및 주간 현황 한 줄 브리핑
## 3. 📊 공정표: 시작일 순 정렬 및 간트 차트 상단 년-월 표시
## 4. ⚙️ 관리: 주간 현황 업데이트 + 개별 공정 상태/진행률 수정 기능 통합
...
# 1. 페이지 설정
st.set_page_config(page_title="PM 통합 공정 관리 v0.5.4", page_icon="🏗️", layout="wide")
            ...
            if not df_raw.empty:
                ...
                with st.form("edit_task_form"):
                    ...
                    if st.form_submit_button("공정 정보 업데이트"):
                        # E열(진행상태), F열(비고), G열(진행률) 업데이트
                        target_ws.update(f"E{idx+2}:G{idx+2}", [[new_stat, new_memo, new_pct]])
                        st.success(f"'{row_data['구분']}' 업데이트 성공!")
                        time.sleep(1)
                        st.rerun()
            else:
                st.info("수정할 공정 데이터가 없습니다.")
