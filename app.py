...
def view_project_detail(sh, pjt_list):
    ...
    if selected_pjt != "선택":
        ...
        with col_pm2: 
            st.write("")
            # [수정] 들여쓰기 오류 해결 (with 블록 안으로 정확히 정렬)
        ...
        with tab3:
            ...
            try:
                h_ws = safe_api_call(sh.worksheet, 'weekly_history'); h_df = pd.DataFrame(safe_api_call(h_ws.get_all_records))
                if not h_df.empty:
...
if check_login():
    ...
    if client:
        try:
            ...
            elif menu == "📊 PPT 자동 생성": ppt_engine.view_ppt_generator(DEFAULT_TEMPLATE) # 외부 엔진 호출
            
            if st.sidebar.button("로그아웃"): st.session_state.logged_in = False; st.rerun()
        except Exception as e: st.error(f"서버 접속 지연... {e}")
