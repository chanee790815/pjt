...
# 1. 페이지 설정
st.set_page_config(page_title="PM 통합 공정 관리 v4.1.8", page_icon="🏗️", layout="wide")
...
st.markdown("""
    ...
    <div class="footer">시스템 상태: 정상 (v4.1.8) | 엑셀 마스터 대시보드 자동화 지원</div>
...
def view_project_admin(sh, pjt_list):
    ...
    t1, t2, t3, t4, t5 = st.tabs(["➕ 등록", "✏️ 수정", "🗑️ 삭제", "🔄 엑셀 동기화", "📥 마스터 다운로드"])
    
    with t4:
        st.markdown("#### 🔄 엑셀 파일 업로드 & 구글 시트 동기화")
        target = st.selectbox("업데이트 프로젝트", ["선택"] + pjt_list)
        file = st.file_uploader("엑셀 파일(.xlsx, .xlsm)", type=['xlsx', 'xlsm'])
        if target != "선택" and file and st.button("동기화 실행"):
    ...
    with t5:
        st.info("💡 모든 현장의 상세 공정표와 '주간업무 히스토리'를 하나의 파일로 병합하여 다운로드합니다.")
        if st.button("📚 통합 마스터 엑셀 일괄 생성", type="primary", use_container_width=True):
            with st.spinner("서버에서 모든 데이터를 병합하는 중..."):
                output = io.BytesIO()
                ...
                st.download_button("📥 통합 리포트 파일 받기", output.getvalue(), f"PMO_Total_Master_{datetime.date.today()}.xlsx", use_container_width=True)
...
if check_login():
    ...
    if client:
        try:
            ...
            menu = st.sidebar.radio("메뉴", ["통합 대시보드", "리스크 현황", "프로젝트 상세", "프로젝트 설정"])
        ...
        except APIError: st.error("구글 시트 접근 권한이 없거나 파일명이 잘못되었습니다.")
