...
def navigate_to_project(p_name):
    st.session_state.selected_menu = "🏗️ 프로젝트 상세"
    ...
    if st.button("🚀 PPT 즉시 생성 시작"):
        ...
        if html_up and final_temp:
            try:
                ...
                # Slide 4~7 생략 (로직 동일 구현)
                ppt_out = io.BytesIO(); prs.save(ppt_out)
                st.success("✅ 생성 완료!"); st.download_button("📥 PPT 다운로드", ppt_out.getvalue(), file_name="신성이엔지_운영전략.pptx")
...
def view_solar(sh):
    ...
    try:
        ...
        if not f_df.empty:
            ...
            st.plotly_chart(fig, use_container_width=True)
    except: st.warning("Solar_DB 시트를 확인해 주세요.")
...
def view_kpi(sh):
    ...
    except: st.warning("KPI 시트를 확인해 주세요.")
...
def view_project_admin(sh, pjt_list):
    ...
    with t3:
        if st.button("📚 통합 엑셀 백업 생성"):
            ...
            st.download_button("📥 백업 다운로드", out.getvalue(), f"PMO_Backup_{datetime.date.today()}.xlsx")
    ...
    if client:
        try:
            ...
            menu = st.sidebar.radio("원하시는 기능을 선택하세요", 
                                    ["📊 통합 대시보드", "🏗️ 프로젝트 상세", "☀️ 일 발전량 분석", "📉 경영지표(KPI)", "⚙️ 마스터 설정", "📊 PPT 자동 생성"], 
                                    key="selected_menu")
