...
with tab1:
    ...
    if not df_raw.empty:
        try:
            ...
            # [수정] 시작일 빠른 순으로 전체 데이터 정렬 (새 업무 추가 시 자동 정렬됨)
            ...
            # [핵심] Y축 표시 순서를 정렬된 데이터의 순서 그대로 리스트화
            ...
            fig.update_layout(
                ...
                yaxis=dict(
                    autorange="reversed", # 리스트의 첫 번째(빠른 날짜)가 맨 위로 오게 함
                    showgrid=True, 
        ...
        # [중요] try-except 블록 밖에서 구분선을 그어 문법 오류 방지
        st.divider()
        st.write("📋 상세 데이터 목록")
