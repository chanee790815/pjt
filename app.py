...
# ---------------------------------------------------------
# [SECTION 1] 백엔드 엔진 & 유틸리티 (v4.5.22 로직)
# ---------------------------------------------------------
...
# 2. 프로젝트 상세 (v4.5.22 로직 완벽 복구 및 IndentationError 해결)
def view_project_detail(sh, pjt_list):
    ...
    if sel_p != "선택":
        ...
        with c_pm1: 
            new_pm = st.text_input("프로젝트 담당 PM (H2 셀)", value=pm)
        with c_pm2: 
            st.write("")
            if st.button("PM 성함 저장"): 
                safe_api_call(ws.update, 'H2', [[new_pm]])
                st.success("PM 업데이트 완료!")
        ...
        with tab1:
            try:
                ...
                if not cdf.empty:
                    ...
                    fig = go.Figure(go.Bar(base=cdf['시작일'], x=cdf['duration'], y=[cdf['대분류'].tolist(), cdf['구분_고유'].tolist()], orientation='h', marker=dict(color=cdf['진행률'], colorscale='RdYlGn', cmin=0, cmax=100, showscale=True)))
        ...
        with tab2:
            try:
                ...
                if not sdf.empty:
                    ...
                    st.plotly_chart(go.Figure(data=[go.Scatter(x=d_range, y=p_trend, mode='lines+markers', name='계획'), go.Scatter(x=[datetime.date.today()], y=[sdf['진행률'].mean()], mode='markers', name='실적', marker=dict(size=12, color='red', symbol='star'))]).update_layout(title="진척률 추이 (S-Curve)"), use_container_width=True)
            except: pass
        with tab3:
            ...
            try:
                ...
                if not h_data.empty:
                    p_match = h_data[h_data['프로젝트명'].astype(str).str.strip() == sel_p.strip()]
            ...
            with st.form("weekly_sync_form"):
                ...
                if st.form_submit_button("시트 데이터 업데이트 및 이력 저장"):
                    ...
                    try: safe_api_call(h_ws.append_row, [datetime.date.today().strftime("%Y-%m-%d"), sel_p, in_t, in_n, st.session_state.user_id])
                    except: pass
        ...
        if st.button("💾 변경사항 전체 저장"):
            ...
            if e_rows:
                for i, r in enumerate(e_rows):
                    r7 = r[:7]
                    f_data.append(r7 + ([pm, in_t, in_n] if i == 0 else [pm, "", ""]))
            else: f_data.append([""]*7 + [pm, in_t, in_n])
            safe_api_call(ws.clear); safe_api_call(ws.update, 'A1', f_data); st.success("저장 성공!"); st.rerun()
    ...
    if st.button("🚀 PPT 즉시 생성 시작"):
        ...
        if h_up and f_temp:
            try:
                ...
                if s3:
                    sld = prs.slides.add_slide(L5); fill_placeholders_optimized(sld, "2. 핵심 역량: 계통 인프라 기술 자산화")
                ...
                if s4:
                    sld = prs.slides.add_slide(L5); fill_placeholders_optimized(sld, "3. AI 기반 에너지 락인(Lock-in) 전략")
                    rect = sld.shapes.add_shape(MSO_SHAPE.RECTANGLE, W*0.06, H*0.3, W*0.65, H*0.55); rect.fill.solid(); rect.fill.fore_color.rgb = RGBColor(240, 253, 244); rect.line.color.rgb = COLOR_MAIN
                    tb = sld.shapes.add_textbox(W*0.08, H*0.33, W*0.6, H*0.5)
                    for item in s4.select(".ai-tag, h3, li, p"): text = get_clean_text(item); 
                    if text: p = tb.text_frame.add_paragraph(); p.text = text; p.font.size = Pt(16); p.space_after = Pt(10)
                s5 = soup.select_one("#slide5")
                if s5:
                    sld = prs.slides.add_slide(L5); fill_placeholders_optimized(sld, "4. PM팀 핵심 실무 그룹 R&R")
                    tiles = s5.select(".tile"); cols = 3; card_w = (W*0.88)/cols; card_h = H*0.25
                    for i, tile in enumerate(tiles): r, c = i // cols, i % cols; create_styled_card(sld, W*0.06+(c*(card_w+Inches(0.1))), H*0.3+(r*(card_h+Inches(0.1))), card_w, card_h, get_clean_text(tile.find("h3")), get_clean_text(tile.find("p")))
                ...
                if s7:
                    sld = prs.slides.add_slide(L5); fill_placeholders_optimized(sld, "Vision 2026")
...
def view_project_admin(sh, pjt_list):
    st.title("⚙️ 마스터 관리")
    t1, t2, t3, t4, t5 = st.tabs(["➕ 등록", "✏️ 수정", "🗑️ 삭제", "🔄 업로드", "📚 백업"])
    with t1:
