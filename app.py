...
# 1. 페이지 설정
st.set_page_config(page_title="PM 통합 공정 관리 v4.5.3", page_icon="🏗️", layout="wide")
...
st.markdown("""
    ...
    <div class="footer">시스템 상태: 정상 (v4.5.3) | 지역별/연도별 일조 분석 엔진 탑재</div>
    ...
    if selected_pjt != "선택":
        ...
        with tab2:
            ...
            if not sdf.empty:
                ...
                fig_s.add_trace(go.Scatter(x=[datetime.date.today().strftime("%Y-%m-%d")], y=[a_prog], mode='markers', name='실적', marker=dict(size=12, color='red', symbol='star')))
                st.plotly_chart(fig_s, use_container_width=True)

        with tab3:
            h_ws = sh.worksheet('weekly_history')
            with st.form("weekly_entry"):
                in_this = st.text_area("금주 업무")
                in_next = st.text_area("차주 업무")
...
def view_solar(sh):
    ...
    try:
        ...
        st.error("Solar_DB 시트를 찾을 수 없습니다."); return
    ...
    if not df_db.empty and sel_locs:
        ...
        for loc in sel_locs:
            ...
            if not loc_df.empty:
                ...
                fig = px.line(m_avg, x='월', y='발전시간', color='연도', markers=True, title=f"📍 {loc} 지점 연도별 추이", line_shape="spline")
                fig.update_layout(xaxis=dict(tickmode='linear', tick0=1, dtick=1))
                st.plotly_chart(fig, use_container_width=True)

        st.subheader("📋 최근 수집 로그")
        st.dataframe(df_db.sort_values('날짜', ascending=False).head(20), use_container_width=True)
...
def view_project_admin(sh, pjt_list):
    st.title("⚙️ 마스터 관리")
    t1, t4, t5 = st.tabs(["➕ 등록", "🔄 엑셀 업로드", "📥 다운로드"])
    with t1:
        new_name = st.text_input("신규 프로젝트명")
        if st.button("생성") and new_name:
    ...
    with t5:
        if st.button("📚 통합 마스터 생성"):
            output = io.BytesIO()
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                for p in pjt_list:
                    pd.DataFrame(sh.worksheet(p).get_all_records()).to_excel(writer, index=False, sheet_name=p[:31])
            st.download_button("📥 다운로드", output.getvalue(), f"PMO_Master_{datetime.date.today()}.xlsx")
