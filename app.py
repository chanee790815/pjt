...
def render_print_button():
    components.html("""
        ...
        <button onclick="printApp()" style="float:right; padding:6px 14px; border-radius:6px; border:1px solid #dee2e6; cursor:pointer;">🖨️ PDF 저장 / 인쇄</button>
        """, height=40)
    ...
    with st.spinner("데이터 분석 중..."):
        for p_name in pjt_list:
            try:
                ...
                dashboard_data.append({"p_name": p_name, "pm_name": pm, "this_w": this_w, "next_w": next_w, "avg_act": avg_act, "avg_plan": avg_plan})
            except: pass
...
# 2. 프로젝트 상세 (Gantt, S-Curve, Editor 완벽 복원)
def view_project_detail(sh, pjt_list):
    st.title("🏗️ 프로젝트 상세 관리")
    render_print_button()
    sel_p = st.selectbox("현장 선택", ["선택"] + pjt_list, key="selected_pjt")
    if sel_p != "선택":
        ...
        with tab3:
            with st.form("weekly_form"):
                ...
                if st.form_submit_button("업데이트 및 이력 저장"):
        ...
        if st.button("💾 변경사항 전체 저장"):
            ...
            for i, r in enumerate(rows):
                r_ext = r + ([pm, in_t, in_n] if i==0 else [pm, "", ""])
                full_data.append(r_ext)
            ...
            try:
                ...
                if s1:
                    ...
                    for shp in slide.placeholders:
                        if shp.placeholder_format.type == 1: shp.text = get_clean_text(s1.find('h1'))
                        elif shp.placeholder_format.type == 2: shp.text = get_clean_text(s1.find('p'))
                ...
                if s2:
                    ...
                    for i, card in enumerate(cards):
                        r, c = i // cols, i % cols
                        create_styled_card(slide, W*0.06+(c*(W*0.22)), H*0.3+(r*(card_h+Inches(0.15))), W*0.21, card_h, get_clean_text(card.find(class_="label")), get_clean_text(card.find(class_="val")), is_kpi=True)
...
def view_solar(sh):
    ...
    try:
        ...
        if not f_df.empty:
            ...
            m2.metric("평균 일사량", f"{f_df['일사량합계'].mean():.2f}")
...
def view_project_admin(sh, pjt_list):
    ...
    with t3:
        if st.button("📚 전체 백업 생성"):
            out = io.BytesIO()
            with pd.ExcelWriter(out, engine='openpyxl') as writer:
                for p in pjt_list: pd.DataFrame(safe_api_call(sh.worksheet, p).get_all_records()).to_excel(writer, index=False, sheet_name=p[:31])
    ...
    if client:
        try:
            ...
            menu = st.sidebar.radio("원하시는 기능을 선택하세요", 
                                    ["📊 통합 대시보드", "🏗️ 프로젝트 상세", "📊 PPT 자동 생성", "☀️ 일 발전량 분석", "📉 경영지표(KPI)", "⚙️ 마스터 설정"], 
                                    key="selected_menu")
            
            if menu == "📊 통합 대시보드": view_dashboard(sh, pjt_list)
            elif menu == "🏗️ 프로젝트 상세": view_project_detail(sh, pjt_list)
            elif menu == "📊 PPT 자동 생성": view_ppt_generator()
            elif menu == "☀️ 일 발전량 분석": view_solar(sh)
            elif menu == "📉 경영지표(KPI)": view_kpi(sh)
            elif menu == "⚙️ 마스터 설정": view_project_admin(sh, pjt_list)
            
            if st.sidebar.button("로그아웃"): st.session_state.logged_in = False; st.rerun()
        except Exception as e: st.error(f"접속 지연 중... ({e})")
