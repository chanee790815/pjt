...
with tab1:
    ...
    if not df_raw.empty:
        try:
            ...
            # 5. 차트 레이아웃 최적화 (상단 년월, 실선 구분)
            fig.update_layout(
                plot_bgcolor="white",
                xaxis=dict(
                    side="top",                # 날짜 상단 배치
                    showgrid=True,             # 월별 세로 실선
                ...
                yaxis=dict(
                    autorange="reversed",      # 상단이 시작일이 빠른 항목이 오도록 반전
                    showgrid=True,
...
with tab2:
    ...
    with st.form("input_form"):
        c1, c2 = st.columns(2)
        in_start = c1.date_input("시작일(마일스톤은 해당일)", datetime.date.today())
        in_end = c2.date_input("종료일", datetime.date.today() + datetime.timedelta(days=30))
        
        c3, c4 = st.columns(2)
        in_dae = st.selectbox("대분류", ["인허가", "설계/조사", "계약", "토목공사", "건축공사", "송전선로", "변전설비", "전기공사", "준공", "MILESTONE"])
        in_gubun = c4.text_input("구분", placeholder="예: 착공, 종합준공, MTR 선발주 등")
        
        c5, c6 = st.columns(2)
        in_status = c5.selectbox("진행상태", ["예정", "진행중", "완료", "지연"])
        in_note = c6.text_input("비고")
        
        if st.form_submit_button("저장하기 💾", use_container_width=True):
            save_gubun = in_gubun.strip() if in_gubun.strip() != "" else "내용 없음"
            sheet.append_row([str(in_start), str(in_end), in_dae, save_gubun, in_status, in_note])
            st.success("✅ 일정이 성공적으로 저장되었습니다!"); time.sleep(1); st.rerun()
