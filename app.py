...
st.markdown("""
    ...
    <div class="footer">시스템 상태: 정상 (v4.5.31) | 전체 기능 보존 및 PPT 생성 기능 통합</div>
...
# ---------------------------------------------------------
# [SECTION 1] 백엔드 엔진 & 유틸리티 (원본 로직 보존)
# ---------------------------------------------------------
...
def calc_planned_progress(start, end, target_date=None):
    ...
    try:
        ...
        return min(100.0, max(0.0, ((target_date - s).days / total) * 100))
    except: return 0.0
...
def create_styled_card(slide, left, top, width, height, title, body, is_kpi=False):
    ...
    if is_kpi: p2.font.bold = True; p2.font.color.rgb = COLOR_MAIN_PPT; p2.alignment = PP_ALIGN.CENTER; p2.font.size = Pt(18)
    else: p2.font.size = Pt(11)

# ---------------------------------------------------------
# [SECTION 2] 뷰(View) 함수 (v4.5.22 원본 기능 완전 복구)
# ---------------------------------------------------------
...
def view_dashboard(sh, pjt_list):
    ...
    normal_cnt, delay_cnt, done_cnt = len([d for d in filtered if d['status_ui'] == "🟢 정상"]), len([d for d in filtered if d['status_ui'] == "🔴 지연"]), len([d for d in filtered if d['status_ui'] == "🔵 완료"])
        ...
        for idx, d in enumerate(filtered):
            with cols[idx % 2]:
                with st.container(border=True):
                    ...
                    with h_col2: st.button("🔍 상세", key=f"go_{d['p_name']}", on_click=navigate_to_project, args=(d['p_name'],), use_container_width=True)
...
# 2. 프로젝트 상세 (v4.5.22 정밀 간트차트 및 I2/J2 이력 저장 완벽 보존)
def view_project_detail(sh, pjt_list):
    ...
    if selected_pjt != "선택":
        ...
        data = safe_api_call(ws.get_all_values)
        pm, this_val, next_val = "", "", ""
        if len(data) > 0:
            header = data[0][:7]
            df = pd.DataFrame([r[:7] for r in data[1:]], columns=header) if len(data) > 1 else pd.DataFrame(columns=header)
            if len(data) > 1 and len(data[1]) > 7: pm = str(data[1][7]).strip()
        ...
        with c_pm1: new_pm = st.text_input("프로젝트 담당 PM (H2 셀)", value=pm)
        with c_pm2: 
            st.write("")
            if st.button("PM 성함 저장"): 
                safe_api_call(ws.update, 'H2', [[new_pm]]); st.success("PM 업데이트 완료!")
        st.divider()
        tab1, tab2, tab3 = st.tabs(["📊 간트 차트", "📈 S-Curve 분석", "📝 주간 업무 보고"])
        with tab1: # v4.5.22 정밀 간트차트 렌더링
            try:
        ...
        with tab2:
            ...
            if not sdf.empty:
                ...
                fig_s = go.Figure(data=[go.Scatter(x=d_range, y=p_trend, mode='lines+markers', name='계획'), go.Scatter(x=[datetime.date.today()], y=[sdf['진행률'].mean()], mode='markers', name='실적', marker=dict(size=12, color='red', symbol='star'))]).update_layout(title="진척률 추이 (S-Curve)")
                st.plotly_chart(fig_s, use_container_width=True)
        with tab3: # 주간보고 저장 및 이력 누적 (원본 유지)
            ...
            with st.form("weekly_sync_form"):
                ...
                if st.form_submit_button("시트 데이터 업데이트 및 이력 저장"):
                    ...
                    st.success("데이터 및 이력이 성공적으로 저장되었습니다!"); time.sleep(1); st.rerun()
        st.divider(); st.subheader("📝 상세 공정표 편집 (A~G열 전용)")
        edited = st.data_editor(df, use_container_width=True, num_rows="dynamic")
        if st.button("💾 변경사항 전체 저장"):
            full_data = [edited.columns.tolist()[:7] + ["PM", "금주", "차주"]]
            rows = edited.fillna("").astype(str).values.tolist()
            if rows:
                for i, r in enumerate(rows):
                    r7 = r[:7]
                    full_data.append(r7 + ([new_pm, in_this, in_next] if i == 0 else [new_pm, "", ""]))
            else: full_data.append([""]*7 + [new_pm, in_this, in_next])
            safe_api_call(ws.clear); safe_api_call(ws.update, 'A1', full_data); st.success("공정표가 완벽하게 저장되었습니다!"); time.sleep(1); st.rerun()
...
def view_project_admin(sh, pjt_list):
    ...
    with t5:
        if st.button("📚 통합 백업 생성"):
            ...
            st.download_button("📥 백업 받기", out.getvalue(), f"Backup_{datetime.date.today()}.xlsx")

# 6. 신규 추가: PPT 자동 생성기 (이사님의 최신 로직 반영)
...
if check_login():
    ...
    if client:
        try:
            ...
            menu = st.sidebar.radio("원하시는 기능을 선택하세요", ["통합 대시보드", "프로젝트 상세", "일 발전량 분석", "경영지표(KPI)", "마스터 설정", "📊 PPT 자동 생성"], key="selected_menu")
        ...
        except Exception as e: st.error(f"서버 지연 중... {e}")
