...
if check_login():
    ...
    if client:
        try:
            ...
            q = st.query_params.get("menu")
            if q and q in MENU_OPTIONS:
                st.session_state.selected_menu = q
            
            st.sidebar.title("📁 PMO 메뉴")
            menu = st.sidebar.radio("메뉴 선택", MENU_OPTIONS, key="selected_menu")
            
            # 상단 가로 메뉴 (탭 스타일, 본문과 자연스럽게 연결)
            current = st.session_state.selected_menu
            nav_items = []
            for name in MENU_OPTIONS:
                if name == current:
                    nav_items.append(f'<span class="pmo-top-nav-item active">● {name}</span>')
                else:
                    nav_items.append(f'<a class="pmo-top-nav-item" href="?menu={quote(name)}">{name}</a>')
            st.markdown('<div class="pmo-top-nav-row">' + " ".join(nav_items) + "</div>", unsafe_allow_html=True)
