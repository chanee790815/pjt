...
# --- [UI] 스타일 (v4.5.22 원본 디자인 100% 복원) ---
COLOR_MAIN = RGBColor(16, 185, 129)
...
# ---------------------------------------------------------
# [SECTION 3] 뷰(View) 함수들 (v4.5.22 원본 소스 복원)
# ---------------------------------------------------------
    ...
    if sel_p != "선택":
        ...
        with tab1:
            try:
                ...
                if not cdf.empty:
                    ...
                    fig = go.Figure(go.Bar(base=cdf['시작일'], x=cdf['duration'], y=[cdf['대분류'], cdf['구분_고유']], orientation='h', marker=dict(color=pd.to_numeric(cdf['진행률'], errors='coerce').fillna(0), colorscale='RdYlGn', cmin=0, cmax=100, showscale=True)))
        ...
        with tab2:
            ...
            if not sdf.empty:
                ...
                fig_s.add_trace(go.Scatter(x=[datetime.date.today()], y=[pd.to_numeric(sdf['진행률'], errors='coerce').fillna(0).mean()], mode='markers', name='실적', marker=dict(size=12, color='red', symbol='star')))
        ...
        with tab3:
            ...
            try:
                ...
                if not h_data.empty:
                    ...
                    if not p_m.empty:
                        latest = p_m.iloc[-1]
                        st.markdown(f"<div class='history-box'><b>최종 보고일:</b> {latest.get('날짜')}<br><br><b>금주:</b> {latest.get('금주업무')}<br><b>차주:</b> {latest.get('차주업무')}</div>", unsafe_allow_html=True)
            except: pass
    ...
    if st.button("🚀 PPT 즉시 생성 시작"):
        ...
        if html_up and final_temp:
            try:
                ...
                # Slide 1~7 생성 로직 (KPI 8개 그리드 및 인원수 동적 추출 포함)
                ...
                st.success("✅ 생성 완료!"); st.download_button("📥 PPT 다운로드", ppt_out.getvalue(), file_name="신성이엔지_PM전략.pptx")
            except Exception as e: st.error(f"오류: {e}")

# 4. 발전량 분석, KPI, 마스터 설정 (v4.5.22 로직 원복)
def view_solar(sh):
    ...
    try:
        ...
        if not f_df.empty:
            st.plotly_chart(go.Figure(data=[go.Bar(x=f_df['날짜'], y=f_df['일사량합계'], name='일사량'), go.Scatter(x=f_df['날짜'], y=f_df['발전시간'], name='발전시간', yaxis='y2')]).update_layout(yaxis2=dict(overlaying='y', side='right')), use_container_width=True)
    except: pass
...
def view_project_admin(sh, pjt_list):
    ...
    with t1:
        ...
        if st.button("생성") and new_n:
            ...
            safe_api_call(ws.append_row, ["시작일", "종료일", "대분류", "구분", "진행상태", "비고", "진행률", "PM", "금주", "차주"]); st.success("완료!"); st.rerun()
    with t3:
